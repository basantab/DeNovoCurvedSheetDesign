...
topol = "L[1-1]H[17-17]L[2-2]H[7-7]L[5-5]E[4-4]L[2-2]E[4-4]L[1-1]H[13-13]L[1-1]E[12-12]L[2-2]E[12-12]L[2-2]E[14-14]L[2-2]E[8-8]L2-2]H[8-8]L[1-1]"
...
bulges =  {'E3':7,'E6':3} # Specific definition. Bulges are located at positions defined by the user
...
for comb in combinations:
	...
	if args.n:
		r_num = args.n
	filename = 'r%i-all_rev-'%(r_num) ; strand=0 ; resnum=0
	...
	for j,bulcomb in enumerate(bcombinations):
			...
			AddSegmentToBlueprint(refblue = 'bp', segments = ['H2','L3','E1','L4','E2','L5','H3','L6'], \
					      ...
					      hs_pairing='HSSTRIPLET 2,1-2;1,5-6',seg_abego={'L3':'BBAAB','L4':'EA','L5':'E','L6':'B'}) # Added B to L6
			# step 5
			AddSegmentToBlueprint(refblue = 'bp', segments = ['L10','H4'], blue0 = 'bp4', \
                                              ...
                                              hs_pairing="HSSTRIPLET 4,3-4", hh_pairing="HHPAIR 2-3.P", seg_abego={'L10':'BB'} )
			# step6
			AddSegmentToBlueprint(refblue = 'bp', segments = ['H1','L2'], blue0 = 'bp5', \
			...
			# RESFILE
			if args.resfile:
				os.system( 'python ../manual_resfile.py bp5.b %s' %( args.resfile ) )
			...
			#original twist: 35.0 5.0 original bend: 55.0 5.0
                        st1 = RegularStrandCurvature(level=2,blueprint=blue,strand='E1',\
                        global_bend=55.0,global_bend_tol=5.0,\
			global_twist=35.0,global_twist_tol=5.0,\
			constraint_type='bounded' )

                        st2 = RegularStrandCurvature(level=2,blueprint=blue,strand='E2',\
			global_twist=35.0,global_twist_tol=5.0,\
                        global_bend=55.0,global_bend_tol=5.0,constraint_type='bounded' )
			'''
			st1 = RegularStrandCurvature(level=2,blueprint=blue,strand='E1',\
			...
			'''
			# These are for the "bulge" connection BA
			...
			st = AngleConstraints(e4c-1,l6n+1,h3c,95,15,"E6H4b") ; fileoutc.write(st);
			'''
			...
			st = PairConstraints(H4c-1,E4c+3,6,2.2,"H4E4") ; fileout.write(st);
                        ...
                        #st = AngleConstraints(E6c-2,L10,H4c,120,5,"E6H4") ; fileout.write(st); # Changed angle form 80 to 110
			...
			H1C_r1 = "-parser:script_vars H1C_r1=%s\n"%(str(h1c-2)) ; flags_out.write(H1C_r1)
			H1C_r2 = "-parser:script_vars H1C_r2=%s\n"%(str(loop6)) ;flags_out.write(H1C_r2)
			H1C_dist = "-parser:script_vars H1C_dist=%s\n"%("12.5") ;flags_out.write(H1C_dist)
			H1Cb_r1 = "-parser:script_vars H1Cb_r1=%s\n"%(str(h1c-1)) ; flags_out.write(H1Cb_r1)
                        H1Cb_r2 = "-parser:script_vars H1Cb_r2=%s\n"%(str(loop6+2)) ;flags_out.write(H1Cb_r2)
                        H1Cb_dist = "-parser:script_vars H1Cb_dist=%s\n"%("9.0") ;flags_out.write(H1Cb_dist)
